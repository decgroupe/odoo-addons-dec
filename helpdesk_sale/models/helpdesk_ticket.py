...
class HelpdeskTicket(models.Model):
    ...
    def action_create_quotation(self):
        ...
        for ticket in self:
            data = {
                'summary':
                    _('Case %s: %s') % (ticket.number, ticket.name),
                'origin':
                    ticket.number,
                'partner_id':
                    ticket.partner_id and ticket.partner_id.id or False,
                'date_order':
                    fields.Date.today(),
            }
            ...
            body = (
                _("Created from helpdesk ticket <a href='web#%s'>%s</a>") % (
                    url_encode(
                        {
                            'id': ticket.id,
            ...
            body = (
                _("New quotation <a href='web#%s'>%s</a> created") % (
                    url_encode(
                        {
                            'id': order.id,
                            'model': 'sale.order',
