...
class Product(models.Model):
    ...
    def _compute_product_available(self):
        ...
        for rec in self:
            ...
            rec.legacy_qty_available = (
                self.with_context(c).get_product_available().get(rec.id, 0.0)
            )
    ...
    def get_product_available(self):
        ...
        if c.get("warehouse", False):
            lot_id = warehouse_obj.read(int(c["warehouse"]), ["lot_stock_id"])[
                "lot_stock_id"
            ][0]
            if lot_id:
                c["location"] = lot_id
        ...
        if c.get("location", False):
            ...
            elif type(c["location"]) in (type(""), type("")):
                location_ids = location_obj.search([("name", "ilike", c["location"])])
            else:
                location_ids = c["location"]
        else:
            location_ids = []
        ...
        uoms = list(map(lambda x: x[2], results)) + list(map(lambda x: x[2], results2))
        if c.get("uom", False):
            uoms += [c["uom"]]
    ...
    def action_update_stock_quant_availability(self):
        ...
        for rec in self:
            ...
            for location_id in location_ids:
                ...
                diff = float_round(
                    r.legacy_qty_available - r.qty_available, precision_rounding=pr
                )
