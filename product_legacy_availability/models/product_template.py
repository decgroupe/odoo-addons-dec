...
class ProductTemplate(models.Model):
    ...
    @api.depends(
        "product_variant_ids",
        "product_variant_ids.legacy_qty_available",
    ...
    def _compute_product_template_available(self):
        ...
        for template in unique_variants:
            template.legacy_qty_available = (
                template.product_variant_ids.legacy_qty_available
            )
            template.legacy_virtual_available = (
                template.product_variant_ids.legacy_virtual_available
            )
            template.legacy_incoming_qty = (
                template.product_variant_ids.legacy_incoming_qty
            )
            template.legacy_outgoing_qty = (
                template.product_variant_ids.legacy_outgoing_qty
            )
    ...
    def action_update_stock_quant_availability(self):
        product_variant_ids = self.with_context(active_test=False).mapped(
            "product_variant_ids"
        )
