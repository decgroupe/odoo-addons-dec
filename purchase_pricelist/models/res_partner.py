...
class Partner(models.Model):
    ...
    def _compute_product_pricelist_purchase(self):
        ...
        res = self.env['product.pricelist']._get_partner_pricelist_purchase_multi(self.ids, company_id=company)
        for p in self:
    ...
    def _inverse_product_pricelist_purchase(self):
        ...
        actual = self.env['ir.property'].get('property_product_pricelist_purchase', 'res.partner', 'res.partner,%s' % self.id)

        # update at each change country, and so erase old pricelist
        if self.property_product_pricelist_purchase or (actual and default_for_country and default_for_country.id != actual.id):
            ...
            self.env['ir.property'].with_context(force_company=self._context.get('force_company', self.env.user.company_id.id)).sudo().set_multi(
                ...
                self._name,
                {self.id: self.property_product_pricelist_purchase or default_for_country.id},
                default_value=default_for_country.id
            )

    def _commercial_fields(self):
        return super(Partner, self)._commercial_fields() + ['property_product_pricelist_purchase']
