...
class mrp_bom(osv.osv):
    ...
    def _bom_explode(self, cr, uid, bom, factor, properties=[], addthis=False, level=0, routing_id=False):
        ...
        if bom.type == 'phantom' and not bom.bom_lines:
            newbom = self._bom_find(
                cr, uid, bom.product_id.id, bom.product_uom.id, properties)

            if newbom:
                res = self._bom_explode(cr, uid, self.browse(cr, uid, [newbom])[
                                        0], factor * bom.product_qty, properties, addthis=True, level=level + 10)
                result = result + res[0]
        ...
        if not phantom:
            ...
            for bom2 in bom.bom_lines:
                res = self._bom_explode(
                    cr, uid, bom2, factor, properties, addthis=True, level=level + 10)
                result = result + res[0]
                result2 = result2 + res[1]
    ...
    def get_cost_price(self, cr, uid, ids, context=None):
        ...
        for id in ids:
            bom_line_ids = self.search(
                cr, uid, [('bom_id', '=', id)], context=context)
...
class stock_move(osv.osv):
    ...
    def _get_status(self, cr, uid, ids, field_names, arg, context=None):
        ...
        # Shipment moves are used to retrieve procurement data (from PTN and
        # REFManager)
        shipment_move_ids = ids + stock_move_obj.search(
            cr, uid, [('move_dest_id', 'in', ids)], context=context)
        procurement_order_ids = procurement_order_obj.search(
            cr, uid, [('move_id', 'in', shipment_move_ids)], context=context)
        ...
        for id in moves.keys():
            ...
            if procure_method <> 'make_to_stock':
                if product_pack:
                    parent_moves_ids = stock_move_obj.search(
                        cr, uid, [('move_dest_id', 'in', move_dest_ids), ('product_id', '!=', product_id.id)], context=context)
                else:
                    parent_moves_ids = stock_move_obj.search(
                        cr, uid, [('move_dest_id', 'in', move_dest_ids)], context=context)

                if parent_moves_ids:
                    parent_moves = stock_move_obj.browse(
                        cr, uid, parent_moves_ids, context=context)
                    parent_moves_wait = [
                        k for k in parent_moves if k.state in ('waiting', 'confirmed', 'assigned')]
                    purchase_moves_exclude = [
                        k.move_dest_id for k in parent_moves_wait if k.move_dest_id]
                    parent_moves_done = [
                        k for k in parent_moves if k.state == 'done' and not k in purchase_moves_exclude]

                    # Exclude all waiting moves where their parent is done
                    parent_moves_done_dest = [
                        k.move_dest_id for k in parent_moves_done if k.move_dest_id]
                    parent_moves_wait2 = [
                        k for k in parent_moves_wait if k not in parent_moves_done_dest]
            ...
            if parent_moves or (procurement and procurement.purchase_id):
                ...
                if purchase_state == 'draft' and procurement_state == 'running':
                    ...
                    message = _('On procurement (partially delivered)') + ' %d/%d' % (
                        len(parent_moves_done), len(parent_moves_wait) + len(parent_moves_done))
                ...
                if purchase_id:
                    dedicated = ('%s (%s: %s)') % (
                        dedicated, purchase_id.name, purchase_id.partner_id.name)
                if production_id:
                    dedicated = ('%s (%s: %s)') % (
                        dedicated, production_id.name, production_id.state)
                # and (not forced or product_pack):
                if parent_moves_done and not parent_moves_wait:
                    received = True
...
class mrp_production(osv.osv):
    ...
    def _make_production_line_procurement(self, cr, uid, production_line, shipment_move_id, context=None):
        ...
        if production_line.product_id.type == 'service':
            procurement_name = '[%s] %s' % (
                production.bom_id.name, production.product_id.name)
        else:
            procurement_name = '%s (%s,%s,%s)' % (procurement_origin, production_line.product_id.type,
                                                  production_line.product_id.procure_method, production_line.product_id.supply_method)
        ...
        wf_service.trg_validate(
            uid, procurement_order._name, procurement_id, 'button_confirm', cr)
    ...
    def _get_sale_dates(self, cr, uid, ids, field_names=None, arg=False, context=None):
        ...
        for prod in self.browse(cr, uid, ids, context=context):
            sale_order_ids = sale_order_obj.search(
                cr, uid, [('name', '=', prod.sale_name)], context=context)
    ...
    def _picked_rate(self, cr, uid, ids, field_names=None, arg=False, context=None):
        ...
        for prod in self.browse(cr, uid, ids, context=context):
            ...
            move_ids = move_obj.search(
                cr, uid, [('move_dest_id', 'in', alt_moves)], context=context)
    ...
    def _get_tested(self, cr, uid, ids, field_names=None, arg=False, context=None):
        ...
        for prod in self.browse(cr, uid, ids, context=context):
            ...
            if total_count > 0:
                res[prod.id]['tested_rate'] = float(
                    ready_count) / float(total_count) * 100
            else:
    ...
    def _get_view_name(self, cr, uid, ids, field_names=None, arg=False, context=None):
        ...
        for prod in self.browse(cr, uid, ids, context=context):
            res[prod.id] = '%s: %s (%s)' % (
                prod.name, prod.product_id.name, prod.bom_id.name)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.has_key('move_prod_id') and vals['move_prod_id'] != False:
            move_prod_id = move_obj.browse(
                cr, uid, vals['move_prod_id'], context)
            while move_prod_id:
                if move_prod_id.sale_line_id and move_prod_id.sale_line_id.order_id:
                    ...
                    vals[
                        'partner_id'] = partner_address_id and partner_address_id.partner_id.id or False
                    break
                ...
                for move in production.move_all_dst_ids:
                    if move.product_id.id <> vals['product_id']:
                        move_obj.write(cr, uid, [move.id], {
                                       'product_id': vals['product_id']}, context=context)

                        # Update next move
                        if move.move_dest_id and move.move_dest_id.product_id.id == move.product_id.id:
                            move_obj.write(cr, uid, [move.move_dest_id.id], {
                                           'product_id': vals['product_id']}, context=context)
    ...
    def run_picked_rate_scheduler(self, cr, uid, ids=None, context=None):
        ...
        if not ids:
            ids = self.search(
                cr, uid, [('state', 'in', ('confirmed', 'picking_except', 'ready', 'in_production'))])
    ...
    def action_produce(self, cr, uid, production_id, production_mode, context=None):
        ...
        if production_mode in ['consume', 'produce']:
            # Consume products (from the picking list to avoid consuming REF
            # Manager moves)
            mrp_moves = []
            if production.picking_id:
                for move in production.picking_id.move_lines:
                    if move.move_dest_id:
                        ...
                        logging.getLogger('mrp.production').error(
                            'action_produce error, not move_dest %s: [%s] %s %s', move.name, move.product_id.default_code or '', move.product_id.name or '', move.picking_id and move.picking_id.name or '')
            ...
            for move_consume in mrp_moves:
                logging.getLogger('mrp.production').info(
                    'action_produce on %s: [%s] %s %s', move_consume.name, move_consume.product_id.default_code or '', move_consume.product_id.name or '', move_consume.picking_id and move_consume.picking_id.name or '')
                move_consume.action_consume(
                    move_consume.product_qty, move_consume.location_id.id, context=context)

            if production_mode == 'produce':
                # Create products
                for move_product in production.move_created_ids:
                    move_product.action_consume(
                        move_product.product_qty, move_product.location_id.id, context=context)
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        if warn:
            self.pool.get('mail.message').schedule_with_attach(
                cr, uid, emailfrom, emails, subject, body, model='mrp.production', reply_to=emailfrom)
        ...
        for production in self.browse(cr, uid, ids, context):
            ...
            if production.move_prod_id:
                proc_ids = proc_obj.search(
                    cr, uid, [('move_id', '=', production.move_prod_id.id)], context=context)
                for procurement in proc_obj.browse(cr, uid, proc_ids, context):
                    break

            project_ids = project_obj.search(
                cr, uid, [('name', 'ilike', 'test')], context=context)
...
class mrp_production_workcenter_line(osv.osv):
    ...
    def _get_sequence(self, cr, uid, context={}):
        ...
        if context and 'production_id' in context:
            productions = self.pool.get('mrp.production').browse(
                cr, uid, context['production_id'])
