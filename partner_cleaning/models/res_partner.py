...
class Partner(models.Model):
    ...
    def action_clean_with_users(self, days):
        ...
        domain = [
            ("create_date", "<=", fields.datetime.today() - timedelta(days=days)),
            ("signup_type", "=", "reset"),
        ]
        inactive_users = self.env["res.users"].search(domain, order="create_date")
        ...
        if partners_to_delete:
            ...
            for id in users_to_delete_raw_ids:
                ...
                _logger.info(
                    "Deleting user %s (%s)", user_id.name_get()[0][1], user_id.login
                )
                user_id.unlink()
