...
class RefReference(models.Model):
    ...
    folder_count = fields.Integer(
        string="Product folder item count",
        copy=False,
    )
    folder_error = fields.Integer(
        string="Product folder error count",
        copy=False,
    )
    folder_warning = fields.Integer(
        string="Product folder warning count",
        copy=False,
    )
    folder_task = fields.Integer(
        string="Product folder task count",
        copy=False,
    )
    ...
    reference_line_ids = fields.One2many(
        comodel_name="ref.reference.line",
        inverse_name="reference_id",
        string="Lines",
    )
    version_ids = fields.One2many(
        comodel_name="ref.version",
        inverse_name="reference_id",
        string="Versions",
        copy=False,
    )
    ...
    def _prepare_product_vals(self, ref_vals):
        ...
        if category_id:
            product_categ_id = (
                self.env["ref.category"].browse(category_id).product_category_id.id
            )
        else:
    ...
    def search_custom2(self, keywords):
        ...
        for key in keywords[0]:
            ...
            if key:
                ...
                add_to(public_code_domain, [("product_id.public_code", "=", key)])
                if use_description:
                    add_to(
                        description_domain, [("product_id.description", "ilike", key)]
                    )
                # A tag must be at least 2 characters
                if len(key) > 2:
                    add_to(tags_domain, [("product_id.tagging_ids.name", "ilike", key)])
        ...
        for line in self.reference_line_ids:
            ...
            if not line.property_fixed and line.property_id.name == "Indice":
                ...
                if not line.value or AUTO_INC_CHAR in line.value:
                    formatted_value = line.property_id.format_int(last_max_value + 1)
                    line.value = AUTO_INC_CHAR + formatted_value

    @api.model
    def _find_last_max_value(self, category_id, reference_line_ids, property_id):
