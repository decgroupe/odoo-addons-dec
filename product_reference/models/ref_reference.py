...
class ref_reference(models.Model):
    ...
    datetime = fields.Datetime('Create date',
                               required=True,
    ...
    def search_custom(self, keywords):
        ...
        for key in keywords[0]:
            ...
            if key:
                ...
                if use_comments:
                    search_comments = self.search([('product.comments',
                                                    'ilike', key)])
                else:
                    search_comments = []

                if len(key) > 2:
                    search_tags = self.search([('product.tagging_ids.name',
                                                'ilike', key)])
                else:
                    search_tags = []
    ...
    def run_material_cost_scheduler(self):
        ...
        for reference in self.browse(ids):
            ...
            if reference.product and reference.product.bom_ids:
                ...
                if bom_id:
                    _logger.info('Compute material cost price for [%s] %s',
            ...
            if not ref_price or (round(ref_price.value, 2) != round(
                    cost_price, 2)):
                ...
                _logger.info('Price did not change for [%s] %s',
                             reference.value, reference.product.name)

        self.generate_material_cost_report(ids)

    @api.model
    def generate_material_cost_report(self,
                                      ids,
                                      date_ref1=False,
                                      date_ref2=False):
        ...
        for reference in self.browse(ids):
            ...
            if (len(price_ids) >= 2):
                ...
                if (round(prices[0].value, 2) > round(
                        prices[1].value, 2)) and ((date_ref1 or date_ref2) or
                                                  (prices[0].date == today)):
        ...
        for content in ref_content:
            body += ('[%s] %s - %d\n') % (content['reference'],
                                          content['product'], content['id'])
            body += ('%s:   %.2f\n') % (content['price1_date'],
                                        content['price1_value'])
            body += ('%s:   %.2f (+%.2f)\n') % (
