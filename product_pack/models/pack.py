...
class product_pack_sale(osv.osv):
    ...
    _columns = {
        ...
        'quantity':
            fields.float('Quantity', required=True),
        'product_id':
            fields.many2one('product.product', 'Product', required=True),
        'product_name':
            fields.related('product_id', 'name', type='char', string='Name'),
        'product_code':
            fields.related(
                'product_id',
                'default_code',
    ...
    _columns = {
        ...
        'fixed_sale_price':
            fields.boolean(
                ...
                help=
                'Mark this field if the price of the pack should be fixed. Do not mark it if the price should be calculated from the sum of the prices of the products in the pack.'
            ),
        'purchase_pack_line_ids':
            fields.one2many(
...
class purchase_order(osv.osv):
    ...
    def _extend_pack_pickings(self, cr, uid, order, order_lines, context=None):
        ...
        for order_line in order_lines:
            ...
            if final_move:
                logging.getLogger('purchase.order').info(
