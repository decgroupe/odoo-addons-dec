...
def _tag_record(self, rec, extra_vals=None):
    """Override `xml_import._tag_record` to handle not found records using their XML-ID.
    If an `xid` field is present in XML data, then use it in place of the XML-ID and
    retry the operation."""
    try:
        ...
        if "Cannot update missing record" in exception.args[0]:
            for child in rec.iterchildren("field"):
                if child.get("name") == "xid":
                    ...
                    res = original_tag_record(self, rec, extra_vals)
...
def _test_xml_id(self, xml_id):
    """Override `xml_import._test_xml_id` to ignore XML-IDs from
    `__export__` and `xml_export` modules."""
    if "." in xml_id:
...
def model_id_get(self, id_str, raise_if_not_found=True):
    """Override `xml_import.model_id_get` to handle not found records using
    their XML-ID.
    If the XML-ID is in the format `modelname_id__uuid`, then extract the model name
    and ID from it and return them. A such format is/was generated by Odoo to
    export data, but it becomes unusable when importing data in a restored database
    where the original record ID is the same but has a different or no XML-ID.
    """
    try:
        ...
        if "External ID not found in the system" in exception.args[0]:
            if "." in id_str and "__" in id_str:
                ...
                if module in ("__export__", "xml_export"):
                    ...
                    model, res_id = model_id.rsplit("_", 1)
                    model = model.replace("_", ".")
                    res_id = int(res_id)
                    if model in self.env:
                        instance_id = self.env[model].browse(res_id)
