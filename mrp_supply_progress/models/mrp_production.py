...
class MrpProduction(models.Model):
    ...
    def _compute_supply_progress(self):
        ...
        for rec in self.filtered("move_raw_ids"):
            all_move_ids = rec.move_raw_ids.filtered(lambda x: x.state != "cancel")
            if all_move_ids:
                received_move_ids = all_move_ids.filtered(lambda x: x.received)
                rec.supply_progress = len(received_move_ids) * 100 / len(all_move_ids)

    def action_update_supply_progress(self):
        self._compute_supply_progress()

    def run_supply_progress_update_scheduler(self):
        date = fields.Datetime.to_string(fields.datetime.now() - timedelta(days=365))
    ...
    def _get_stages_ref(self):
        ...
        res.update(
            {
                "supplying": self.env.ref("mrp_supply_progress.stage_supplying"),
                "build_ready": self.env.ref("mrp_supply_progress.stage_build_ready"),
            }
        )
        return res
