...
def parse(content):
    ...
    result["description_short"] = parser_helper.clean(
        tree.xpath('//span[@class="c-product__specs"]/descendant-or-self::*/text()')
    )
    ...
    for tr in rows:
        name = parser_helper.clean(
            tr.xpath('string(./td[@class="label"]/descendant-or-self::*/text())')
        )
        value = parser_helper.clean(
            tr.xpath('string(./td[@class="value"]/descendant-or-self::*/text())')
        )
