...
def parse(content):
    ...
    result["name"] = parser_helper.clean(
        tree.xpath('//h1[@class="bd-ProductCard-title"]/span[@itemprop="name"]/text()')
    )

    raw_json_data = tree.xpath('//section[@id="bd-Anchor-desc"]/@data-product-infos')
    ...
    result["images"] = []
    pictures = tree.xpath('//li[contains(@class, "bd-ProductView-item")]/picture/img')
    print(len(pictures))
    for p in pictures:
            ...
            if next_node.tag == "ul":
                ...
                for li in lis:
                    ...
                    value = parser_helper.clean(
                        li.xpath('string(./div[@class="bd-ProductDetails-val"]/text())')
                    )
                    result.add_description(name, value, "  -")
