...
class GitlabService(models.AbstractModel):
    ...
    def add_project_member(self, project_uid, user_uid, access_level=10):
        ...
        status, response, headers, ask_time = self._do_request(
            "/api/v4/projects/%d/members" % (project_uid),
            params,
            self._get_common_headers(),
            "POST",
        )
    ...
    def remove_project_member(self, project_uid, user_uid):
        params = {}
        status, response, headers, ask_time = self._do_request(
            "/api/v4/projects/%d/members/%d" % (project_uid, user_uid),
            params,
            self._get_common_headers(),
            "DELETE",
        )
    ...
    def get_user_memberships(self, user_uid):
        params = {}
        status, response, headers, ask_time = self._do_request(
            "/api/v4/users/%d/memberships" % (user_uid),
            params,
            self._get_common_headers(),
            "GET",
        )
    ...
    def _do_request(
        ...
        if headers is None:
            headers = {}
        _logger.debug(
            "Uri: %s - method : %s - Headers: %s - Params : %s !",
            uri,
            method,
            headers,
            params,
        ...
        try:
            ...
            if method.upper() in PARAMS_METHODS:
                res = requests.request(
                    method.lower(), uri, params=params, headers=headers, timeout=TIMEOUT
                )
            ...
            _logger.exception(
                "Bad GitLab request with params %s : %s !",
                params,
            ...
            raise UserError(
                _("Something went wrong with a request to GitLab: %s") % (message)
            )
        return result
