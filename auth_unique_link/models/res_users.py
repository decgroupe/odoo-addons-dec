...
class ResUsers(models.Model):
    ...
    def _signin_link_retrieve_user(
        ...
        if not user:
            if raise_exception:
                raise UserError(
                    _("Signin link token '%s' is not valid") % token
                )
            return False
        if check_validity and not user.signin_link_valid:
            if raise_exception:
                raise UserError(
                    _("Signin link token '%s' is no longer valid") % token
                )
            return False
        return user
    ...
    def _get_signin_link_expiration_minutes(self):
        ...
        expiration_minutes = int(
            ICP.get_param('auth_unique_link.expiration_minutes')
        )
        ...
        for rec in self:
            ...
            if template:
                template = template.with_context(
                    dbname=self._cr.dbname, lang=lang
                )
