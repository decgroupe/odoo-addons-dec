...
class SoftwareLicensePass(models.Model):
    ...
    license_count = fields.Integer(
        compute="_compute_license_hardware_count",
    ...
    hardware_ids = fields.One2many(
        ...
        copy=False,
        help="Hardware identifiers attached to our licences",
    )
    hardware_group_ids = fields.One2many(
        comodel_name="software.license.hardware.group",
        inverse_name="pass_id",
        string="Hardware Groups",
        copy=False,
    ...
    def action_view_hardwares(self):
        return self.with_context(
            active_test=False
        ).license_ids.hardware_ids.action_view()
    ...
    def _compute_license_hardware_count(self):
    ...
    def _compute_hardware_group_ids(self):
        ...
        for rec in self:
            ...
            for hardware_group_id in rec.hardware_group_ids:
                ...
                if couple in local_data:
                    local_data.remove(couple)
                else:
                    to_remove |= hardware_group_id
            to_remove.unlink()
            for name, device_fqdn in local_data:
                self.env["software.license.hardware.group"].create(
                    {
                        "name": name,
            ...
            rec.hardware_group_count = len(rec.hardware_group_ids)
