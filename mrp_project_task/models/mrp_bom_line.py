...
class MrpBomLine(models.Model):
    ...
    def _convert_qty_company_hours(self):
        ...
        if (
            product_uom.id != company_time_uom_id.id
            and product_uom.category_id.id == company_time_uom_id.category_id.id
        ):
            ...
            if (
                uom_hour
                and product_uom.id != uom_hour.id
                and product_uom.category_id.id == uom_hour.category_id.id
            ):
                planned_hours = product_uom._compute_quantity(product_uom_qty, uom_hour)
            else:
