...
class SoftwareApplication(models.Model):
    ...
    def _get_gitlab_project_uids(self):
        return self.mapped("documentation_gitlab_resource_id").mapped(
            "uid"
        ) + self.mapped("resource_ids").mapped(
            "documentation_gitlab_resource_id"
        ).mapped(
            "uid"
        )
    ...
    def write(self, vals):
        need_access_update = (
            "documentation_gitlab_resource_id" in vals or "resource_ids" in vals
        )
        ...
        if need_access_update:
            ...
            diff_project_uids = list(set(pre_project_uids) - set(post_project_uids))
