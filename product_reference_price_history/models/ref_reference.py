...
class RefReference(models.Model):
    ...
    def _run_material_cost_scheduler(self):
        ...
        for rec in self:
            ...
            if rec.product_id and rec.product_id.bom_ids:
                ...
                if bom_id:
                    _logger.info(
                        'Compute material cost price for [%s] %s', rec.value,
                        rec.product_id.name
                    )
                    try:
                        ...
                        _logger.exception(
                            ...
                            rec.value, rec.product_id.name, e
                        )
            ...
            if not ref_price or (
                ...
                _logger.info(
                    'New price stored for [%s] %s', rec.value,
                    rec.product_id.name
                )
            else:
                _logger.info(
                    'Price did not change for [%s] %s', rec.value,
                    rec.product_id.name
                )
        ...
        for rec in self:
            ...
            if len(prices) >= 2:
                if round(prices[0].value, 2) > round(prices[1].value, 2) and (
                    ...
                    line = self._get_reference_report_line(
                        rec, prices, format_prices
                    )
    ...
    def _get_cost_report_default_email(self):
        email = self.env['ir.config_parameter'].sudo().get_param(
            'product_reference_price_history.cost_report_email', ''
        )
        return email
